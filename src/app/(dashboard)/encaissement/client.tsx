...
export function EncaissementCie({ factures }: { factures: F[] }) {
  ...
  const submit = () => {
    ...
    if (!confirm(`Valider l'encaissement de ${selectedFacts.length} facture(s) — Total : ${formatMoney(totalSaisi)} ?`)) return;
    start(() => batchEncaissement(payload));
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3 pb-3 border-b">
        ...
        <Button className="ml-auto" size="sm" disabled={pending || selectedFacts.length === 0}
