...
export async function GET() {
  ...
  const rows = factures.map((f) => [
    ...
    f.statut_part_assureur ?? "", f.date_depot ?? null, f.date_encaissement ?? null,
  ]);

  const buf = await toXlsx([{
    name: "Factures",
    headers: ["Date", "N° Facture", "Assurance", "Montant total", "Part assureur", "Part assureur payée", "Part assuré", "Statut", "Date dépôt", "Date encaissement"],
    rows,
  }]);
